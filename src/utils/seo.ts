...
export function updateMatchSEO(match: Match | null) {
  if (!match) {
    ...
    document.title = "ماتشورا | بث مباشر كأس العالم 2026 - Matchora Live";
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) {
      metaDesc.setAttribute("content", "شاهد مباريات كأس العالم 2026 بث مباشر مجاني على ماتشورا. جميع المباريات بجودة عالية بدون تقطع. Watch FIFA World Cup 2026 free live streaming.");
    }
  ...
  // Set dynamic title
  document.title = `${match.homeTeam.name} vs ${match.awayTeam.name} بث مباشر | ماتشورا`;
  ...
  if (metaDesc) {
    ...
    metaDesc.setAttribute(
      "content", 
      `شاهد مباراة ${match.homeTeam.name} و ${match.awayTeam.name} بث مباشر مجاني على ماتشورا. ${match.competition} - ${match.stadium || 'استاد'} - ${formattedDate}`
    );
  }
