...
export default function SubscribeDialog({ lang, theme }: { lang: 'ar' | 'en'; theme: 'light' | 'dark' }) {
  ...
  useEffect(() => {
    ...
    const hasSubscribed = localStorage.getItem('matchora_yt_subscribed');
    if (!hasSubscribed) {
  ...
  const handleSubscribe = () => {
    window.open('https://youtube.com/@matchora-c6h?si=_fzMJePRzP0EgJrt', '_blank');
    localStorage.setItem('matchora_yt_subscribed', 'true');
    setIsOpen(false);
  };

  const handleClose = () => {
    localStorage.setItem('matchora_yt_subscribed', 'true'); // don't annoy them again to keep it professional
    setIsOpen(false);
  };
          ...
          >
             ...
             <div className="relative z-10 flex flex-col items-center">
                ...
                <p className={cn("text-sm font-medium mb-8 max-w-[260px] leading-relaxed", theme === 'dark' ? 'text-gray-400' : 'text-gray-600')}>
                  {lang === 'ar' 
                    ? 'اشترك في قناة Matchora على يوتيوب واستمتع بمشاهدة المباريات الحصرية بدون انقطاع وبأعلى جودة.'
                    : 'Subscribe to the Matchora YouTube channel and enjoy watching exclusive matches in HD quality.'}
                </p>
